// Functions to Test
const parseInput = require('./parse-input.js');
const { travel, lands, part1 } = require('./part1.js');
const { getHitsAt, findHits, part2 } = require('./part2.js');

// Test Input
const input = 'target area: x=20..30, y=-10..-5';
const parsedInput = { x: [20, 30], y: [-10, -5] };

// Given Test Results
const part2hits = [
  [23, -10],
  [25, -9],
  [27, -5],
  [29, -6],
  [22, -6],
  [21, -7],
  [9, 0],
  [27, -7],
  [24, -5],
  [25, -7],
  [26, -6],
  [25, -5],
  [6, 8],
  [11, -2],
  [20, -5],
  [29, -10],
  [6, 3],
  [28, -7],
  [8, 0],
  [30, -6],
  [29, -8],
  [20, -10],
  [6, 7],
  [6, 4],
  [6, 1],
  [14, -4],
  [21, -6],
  [26, -10],
  [7, -1],
  [7, 7],
  [8, -1],
  [21, -9],
  [6, 2],
  [20, -7],
  [30, -10],
  [14, -3],
  [20, -8],
  [13, -2],
  [7, 3],
  [28, -8],
  [29, -9],
  [15, -3],
  [22, -5],
  [26, -8],
  [25, -8],
  [25, -6],
  [15, -4],
  [9, -2],
  [15, -2],
  [12, -2],
  [28, -9],
  [12, -3],
  [24, -6],
  [23, -7],
  [25, -10],
  [7, 8],
  [11, -3],
  [26, -7],
  [7, 1],
  [23, -9],
  [6, 0],
  [22, -10],
  [27, -6],
  [8, 1],
  [22, -8],
  [13, -4],
  [7, 6],
  [28, -6],
  [11, -4],
  [12, -4],
  [26, -9],
  [7, 4],
  [24, -10],
  [23, -8],
  [30, -8],
  [7, 0],
  [9, -1],
  [10, -1],
  [26, -5],
  [22, -9],
  [6, 5],
  [7, 5],
  [23, -6],
  [28, -10],
  [10, -2],
  [11, -1],
  [20, -9],
  [14, -2],
  [29, -7],
  [13, -3],
  [23, -5],
  [24, -8],
  [27, -9],
  [30, -7],
  [28, -5],
  [21, -10],
  [7, 9],
  [6, 6],
  [21, -5],
  [27, -10],
  [7, 2],
  [30, -9],
  [21, -8],
  [22, -7],
  [24, -9],
  [20, -6],
  [6, 9],
  [29, -5],
  [8, -2],
  [27, -8],
  [30, -5],
  [24, -7],
];

// Test Specs

describe('day17-parse-input: ', () => {
  test('parsing the sample data should equal the handcoded expectation', () => {
    expect(parseInput(input)).toEqual(parsedInput);
  });
});

describe('day17-part1: ', () => {
  describe('travels()', () => {
    test('traveling one step should follow given rules', () => {
      expect(travel([0, 0], [7, 2])).toEqual({
        coords: [7, 2],
        velocity: [6, 1],
      });

      expect(travel([7, 2], [6, 1])).toEqual({
        coords: [13, 3],
        velocity: [5, 0],
      });
    });
  });

  describe('lands()', () => {
    test('given examples should land', () => {
      expect(lands([7, 2], parsedInput).adjustment).toBe(0);
      expect(lands([6, 3], parsedInput).adjustment).toBe(0);
      expect(lands([9, 0], parsedInput).adjustment).toBe(0);
      expect(lands([9, -1], parsedInput).adjustment).toBe(0);
    });

    test('given examples should not land', () => {
      expect(lands([17, -4], parsedInput).adjustment).toBe(-1);
    });
  });

  test('processing sample data should equal 45', () => {
    expect(part1(parsedInput, 10)).toEqual(45);
  });
});

describe('day17-part2: ', () => {
  test('processing sample data should equal 112', () => {
    expect(part2(parsedInput)).toEqual(112);
  });

  describe('getHitsAt()', () => {
    test('it should work with positive numbers', () => {
      expect(getHitsAt(9, parsedInput)).toEqual([
        [6, 9],
        [7, 9],
      ]);
    });

    test('it should work with zero', () => {
      expect(getHitsAt(0, parsedInput)).toEqual([
        [6, 0],
        [7, 0],
        [8, 0],
        [9, 0],
      ]);
    });

    test('it should work with negative numbers', () => {
      expect(getHitsAt(-1, parsedInput)).toEqual([
        [7, -1],
        [8, -1],
        [9, -1],
        [10, -1],
        [11, -1],
      ]);
    });
  });

  describe('findHits()', () => {
    const hits = findHits(parsedInput);
    test.each(part2hits)(
      'coords [%i, %i] should appear in the list',
      (x, y) => {
        expect(hits).toContainEqual([x, y]);
      }
    );
  });
});
